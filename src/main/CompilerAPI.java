...
public class CompilerAPI {
	final Logger logger = Logger.getLogger(CompilerAPI.class.getName()) ;
	...
	public CompilerAPI(IDEFrame ide) {
		this.ide = ide;
	}
	 ...
	 */
	public void doCompilation (String sourceCode){
		this.sourceCode = sourceCode;
		
		/*Creating dynamic java source code file object*/
		SimpleJavaFileObject fileObject = new DynamicJavaSourceCodeObject ("GeneratedCode", sourceCode) ;
		...
		try {
			...
			File path = null;
			if (fileChooser.showSaveDialog(ide) == JFileChooser.APPROVE_OPTION) {
				path = fileChooser.getSelectedFile();	
				Manifest manifest = new Manifest();
				manifest.getMainAttributes().put(Attributes.Name.MANIFEST_VERSION, "1.0");
				manifest.getMainAttributes().put(Attributes.Name.MAIN_CLASS, "GeneratedCode");
				JarOutputStream out = new JarOutputStream(new FileOutputStream(path), manifest);
				for (File file : dir.listFiles()) {
					addFile(file, out);
				}
				out.close();
				JOptionPane.showMessageDialog(ide, "Successfull imported in " + path.getName() + "!");
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private static void addFile(File source, JarOutputStream target) throws IOException
	{
		...
		try{
			...
			String path = source.getPath().replace("\\", "/").replaceAll("out/", "");
